...
void SaveToFile(Node*,char*);
void ReadFromFile(Node**,Node**,char*);
char* trim(char*);
...
int main(void)
{
	...
	Assignment:
	Add following commands to exercise 18.1 or 18.2.
	...
	1) The function "SaveToFile" needs to be added
	2) The function "ReadFromFile" needs to be added
	3) The function "main" needs to be modified to fit the new added functions
	4) The function "showHelp" needs to be modified to fit the new added option
	...
	*/
	...
	while (*pi != 'q')
	{
		...
		case 'h':
			...
			break;
		case 's':
			SaveToFile(head,trim(pi + 1));
			break;
		case 'o':
			ReadFromFile(&head,&tail,trim(pi + 1));
			break;
		case 'q':
			break;
		default:
...
void showHelp(void)
{
	...
	printf("s <filename> : saves the names in the list to a file\n");
	printf("o <filename> : reads names from a file and inserts them into a list\n");
...
void SaveToFile(Node * b,char * s)
{
	...
	{
		printf("This function saves the names in the list to file \"%s\"\n",s);
		while (b->next != NULL)
		{
			fprintf(fd,"%s\n",b->name);
			b = b->next;
...
void ReadFromFile(Node** b,Node** e,char * s)
{
	FILE* fs = fopen(s,"r");
	if (fs == NULL)
	{
	...
	{
		printf("This function reads the names from the file\"%s\" and inserts them into the list\n",s);
		char Name[STRLEN];
		while (fscanf(fs, "%s",Name) > 0)
		{
			add(&Name,b,e);
		}
	}
