...
int main(void)
{
	...
	Use a text editor to create 2 different files that both contain a row of 
	...
	*/
	...
	printf("\n\t----\tDit is oefening 14.9 van 'Cprogrammingfundamentals2020' Pg202\t----\n\n\n\n");
...
void CalculateFile(FILE* fNumbers1, FILE* fNumbers2)
{
	...
	while (ret1 > 0 && ret2 > 0) // We are forced to put the "fscanf" into the "while" condition otherwise we risk to end up in a endless loop
	{
		...
		if (Num1 > Num2)
		{
			do
			{
				printf("\033[31m%3d\033[m |", Num2); // Red
		...
		printf("\033[32m%3d\033[m |", Num1); // Green
		ret1 = fscanf(fNumbers1, "%d%*c", &Num1);
	}
	...
	if (ret2 > 0)
	{
		do
		{
			printf("\033[35m%3d\033[m |", Num2); // Purple
	...
	if (ret1 > 0)
	{
		do
		{
			printf("\033[36m%3d\033[m |", Num1); // Cyan
